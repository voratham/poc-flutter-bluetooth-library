...
import 'package:test_flutter_reactive_ble/src/ble/ble_logger.dart';
...
class DeviceLogTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Consumer<BleLogger>(
    builder: (context, logger, _) => _DeviceLogTab(messages: logger.messages),
  );
}

class _DeviceLogTab extends StatelessWidget {
  const _DeviceLogTab({required this.messages, Key? key}) : super(key: key);
